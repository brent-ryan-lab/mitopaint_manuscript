# Title: umap grid- dim red mitopaint vis (mean per well) v1
# Step: 6.2.2
# R: 4.4.1
# Author: Sarah Franks
...
# plot umap_grid: split by compound, colour by concentration ####
...
plot_umap_compound <- function(data, compound_name) {
  ...
  ggplot() +
    ...
    geom_point(
      data = fg_df,
      aes(x = umap_1, y = umap_2, colour = Concentration),
      size = 1.5
    ) +
...
plots <- purrr::map(
  compound_levels,
  ~ add_fixed_legend_space(plot_umap_compound(data, .x))
)
# arrange into a grid #
...
# save plot ####
ggsave(
  filename = paste0("outputs/figures/umap/", file_name, "_umap_grid.pdf"),
  plot = umap_grid,
  width = plot_width,
  height = plot_height
)
